...
//    window.FK_PRODUCTS        → used by product.html
...
(function () {
    ...
    // Fetch via PHP — direct fetch() to .json is blocked by InfinityFree
    // Use absolute path from root so this works regardless of page depth
    ...
    fetch(_base + 'assets/products.php')
        .then(r => r.json())
        .then(products => {
            window.FK_SEARCH_PRODUCTS = products;          // raw array for search/index
            window.FK_PRODUCTS        = buildFKProducts(products); // map for product.html
            // Dispatch event so pages waiting on data can proceed
